...
async function loadWhitepaper(lang) {
  ...
  try {
    ...
    container.innerHTML = DOMPurify.sanitize(html2);
...
function handleHash() {
  if (location.hash === "#whitepaper") {
    ...
    const wpLang = wpSelect?.value || localStorage.getItem("lang") || currentLang;
...
async function initI18n() {
  ...
  if (wpSelect) {
    wpSelect.addEventListener(
      "change",
      (e) => loadWhitepaper(e.target.value)
    );
  }
}
if (typeof window !== "undefined") {
...
var HCFancyTitle = class extends HTMLElement {
  ...
  render() {
    ...
    h1.setAttribute("data-text", text2);
    root.appendChild(h1);
  }
};
